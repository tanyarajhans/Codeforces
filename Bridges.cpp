...
int vis[100005];
int low[100005]; //in time of lowest ancestor that can be reached 
int in[100005]; //entry time
int timer=0;
...
void dfs(int node, int p){
    ...
    for(int i=0;i<adj[node].size();i++){
        ...
        if(u==p)
            continue;
        if(vis[u]){ //back edge - can never be a bridge
            low[node]=min(low[node], in[u]);
        }
        else{
            dfs(u, node);
            if(low[u]>in[node]) //node <-> v is a bridge
                cout<<node<<"->"<<u<<"is a bridge"<<endl;
