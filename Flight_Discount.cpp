...
void solve(){
    ...
    // {cost, {dest, flight coupon or not}}
    priority_queue<pair<int, pair<int, int> > , vector<pair<int, pair<int, int> > >, greater<pair<int, pair<int, int> > > > pq;
    for(int i=2;i<=n;i++)
    {
        dist[i]=1e17; //cost w/o discount
        disc[i]=1e17; //cost w discount
    }
    pq.push(mp(0, mp(1, 0)));
    ...
    while(!pq.empty()){
        ...
        for(auto e: adj[dest]){
            ...
            if(flight==0){
                if(dist[v]>c+cost){
                    dist[v]=c+cost;
                    pq.push(mp(dist[v], mp(v, 0)));
                }
                if(disc[v]>c/2+cost){
                    disc[v]=c/2+cost;
                    pq.push(mp(disc[v], mp(v, 1)));
                }
